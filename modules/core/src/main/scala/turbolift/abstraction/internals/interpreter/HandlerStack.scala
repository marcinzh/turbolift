...
import turbolift.abstraction.internals.handler.{PrimitiveHandler, Lifting2, Context}
...
private object HandlerStackCases {
  ...
  sealed trait CanDecode[Q[_]] extends CanLift[Q, Q, Id] {
    final override def lifting = Lifting2.identity[Q]
    final override def canDecode: CanDecode[Q] = this

    def makeDecoder[P[_]: MonadPar, F[_]](penthouse: Lifting2[P, Q, F]): Signature[P]
  }
  ...
  final case class PushNext[T[_[_], _], O[_], P[_], Q[_], F[_]](
    ...
    override val lifting = Lifting2.compose(commonOps, that.lifting)
  }
  ...
  final case class PushFirst[T[_[_], _], O[_], M[_]: MonadPar](primitive: PrimitiveHandler[T, O]) extends CanDecode[T[M, ?]] { outer =>
    ...
    override def makeDecoder[P[_]: MonadPar, F[_]](penthouse: Lifting2[P, T[M, ?], F]): Signature[P] = {
      val context: primitive.ThisContext[M, P] =
        new Context {
          ...
          override val lifting: Lifting2[Outer, Main, Stash] = penthouse
        }
