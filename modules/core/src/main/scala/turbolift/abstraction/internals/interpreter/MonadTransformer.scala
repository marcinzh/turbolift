...
sealed trait MonadTransformer[T[_[_], _], O[_]] extends HasSignature {
  ...
  def onTransform[M[_]: MonadPar]: MonadPar[T[M, *]]
  def onOperation[M[_], F[_], U](implicit ctx: ThisControl[M, F, U]): Signature[U]

  type ThisControl[M[_], F[_], U] = Control[T, M, F, U]

  @inline final implicit def innerMonad[M[_], F[_], U](implicit ctx: ThisControl[M, F, U]): MonadPar[M] = ctx.innerMonad
  @inline final implicit def stashFunctor[M[_], F[_], U](implicit ctx: ThisControl[M, F, U]): Functor[F] = ctx.lifting.stashFunctor
}


object MonadTransformerCases {
 abstract class Stateless[O[_]: Functor] extends MonadTransformer[Lambda[(`M[_]`, A) => M[O[A]]], O] with InterpreterCases.SaturatedStateless[O] {
  ...
  abstract class Stateful[S, O[_]: Functor] extends MonadTransformer[Lambda[(`M[_]`, A) => S => M[O[A]]], O] with InterpreterCases.UnsaturatedStateful[S, O]{
