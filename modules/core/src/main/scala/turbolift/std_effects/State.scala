...
trait State[S] extends Effect[StateSig[S]] with StateSig[S] {
  ...
  class DefaultHandler extends Unary[S, (S, +?)] {
    def lift[M[+_] : MonadPar, A](ma: M[A]): S => M[(S, A)] = s => ma.map((s, _))

    def flatMap[M[+_] : MonadPar, A, B](tma: S => M[(S, A)])(f: A => S => M[(S, B)]): S => M[(S, B)] =
      s0 => tma(s0).flatMap {
        case (s1, a) => f(a)(s1) 
      }

    def zipPar[M[+_] : MonadPar, A, B](tma: S => M[(S, A)], tmb: S => M[(S, B)]): S => M[(S, (A, B))] =
      s0 => tma(s0).flatMap { 
    ...
    def decode[M[+_] : MonadPar] = new Decode[M] with StateSig[S] {
